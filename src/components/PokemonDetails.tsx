          ...
          <div className="">
            ...
            <div className="flex flex-row gap-2 border-b-1 border-b-zinc-400 my-5 pb-5">
              {data && data.types.map((type) => {
                return (
                  <Link to={`/type/${type.type.name}`} key={type.type.url} className="px-3 py-1 rounded-full bg-blue-100 text-blue-400 capitalize">{type.type.name}</Link>
                )
              })}
            </div>
