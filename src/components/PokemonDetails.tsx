  ...
  return (
    <section className="grid grid-cols-[minmax(12px,_1fr)_4fr_minmax(12px,_1fr)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 col-start-2 col-end-3 ">
        <div>
          <img src={data.sprites.other["official-artwork"].front_default} alt={`Image of ${name}`} className="bg-zinc-300 rounded-b-sm w-full"/>
        </div>
        <div>
          <div className="">
            <p className="text-zinc-400 text-sm">#{data.order}</p>
            <h2 className="text-zinc-950 text-3xl capitalize font-bold">{data.name}</h2>
            <div className="flex flex-row gap-2 border-b-1 border-b-zinc-400 my-5 pb-5">
              {data.types.map((type) => {
                return (
                  <div key={type.type.url} className="px-3 py-1 rounded-full bg-blue-100 text-blue-400 capitalize">{type.type.name}</div>
                )
              })}
            </div>
            ...
            <div className="stats mt-5 pt-5 border-t-1 border-t-zinc-400">
              ...
              <ul className="list-disc ml-13.5 capitalize">
                {data.abilities.map((a) => {
                  return (
                    <li key={a.ability.url}>{formatStat(a.ability.name)}</li>
                  )
                })}
              </ul>
            </div>
