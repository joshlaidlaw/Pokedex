  ...
  return (
    <main className="grid grid-cols-[minmax(12px,_1fr)_4fr_minmax(12px,_1fr)]">
      ...
      <section className="col-start-2 col-end-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 ">
        ...
        {data?.results &&
          data?.results.map((pokemon: { name: string }) => <Card key={pokemon.name} name={pokemon.name} />)}
      </section>
      {data && data?.results?.length > 0 && (
        <div className="col-start-2 col-end-3 col-span-4 my-6 mx-auto flex flex-row justify-center bg-zinc-100  border-1 border-zinc-400 rounded-sm">
          <button
            className="flex content-center justify-center w-8 h-8 disabled:text-zinc-400 border-r-1 border-zinc-400"
          ...
          <div>
            {paginationLength.map((_, index) => {
              ...
              return (
                <button
                  key={pageNumer}
                  className={`w-8 h-8 border-r-1 border-zinc-400 ${currentPage === pageNumer && 'bg-blue-600 text-blue-50'}`}
                  onClick={() => handlePagination(pageNumer)}
                >
                  {pageNumer}
                </button>
              )
          ...
          <button
            className="flex content-center justify-center w-8 h-8"
